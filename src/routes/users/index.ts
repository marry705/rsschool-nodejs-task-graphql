...
const plugin: FastifyPluginAsyncJsonSchemaToTs = async (
          ...
          await fastify.db.users.change(
            follower.id,
            {
              subscribedToUserIds: follower.subscribedToUserIds
                .filter((followerId) => followerId !== deletedUser.id)
            }
          )
        );
  ...
  fastify.post(
    ...
    async function (request, reply): Promise<UserEntity> {
        ...
        equals: request.params.id
			...
			const followerIndex = subscriber.subscribedToUserIds
        .findIndex((follower) => follower === request.body.userId);
  ...
  fastify.post(
    ...
    async function (request, reply): Promise<UserEntity> {
			...
			const followerIndex = unSubscriber.subscribedToUserIds
        .findIndex((follower) => follower === request.body.userId);

			const subscriberIndex = candidate.subscribedToUserIds
        .findIndex((subscriber) => subscriber === request.params.id);
			...
			const updatedUser = await fastify.db.users.change(
        request.params.id,
        { subscribedToUserIds: unSubscriber.subscribedToUserIds
            .filter((follower) => follower != request.body.userId)
        }
      );

			await fastify.db.users.change(
        request.body.userId,
        { subscribedToUserIds: candidate.subscribedToUserIds
